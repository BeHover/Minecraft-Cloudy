...
class LandsManagementService
{
    ...
    public function getLandsByUser(User $user, string $locale): JsonResponse
    {
        if ($locale !== "en_EN" && $locale !== "ru_RU") {
            return new JsonResponse([
                "message" => "The selected language is not supported by the application."
        ...
        foreach ($lands as $land) {
            $landData = $this->getLandData($land);

            if ($user->getUsername() === $landData['owner'])
            {
                $data[] = $landData;
            }
        }
    ...
    private function getLandData(Lands $land) : array
    {
        ...
        return [
            ...
            "owner" => $owner,
            "type" => $land->getType(),
            "created" => date("d-m-Y, H:i", (int)($land->getCreated() / 1000)),
